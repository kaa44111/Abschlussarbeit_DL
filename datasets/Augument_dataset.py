...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        try:
            ...
            #image = v2.functional.pil_to_tensor(image).to(torch.float16)/ 255.0

            # Laden der Maske für dieses Bild
            mask_name = os.path.join(self.mask_folder, self.mask_files[idx])
            mask = Image.open(mask_name).convert('L')
            #mask = torch.from_numpy(np.array(mask)).unsqueeze(0).to(torch.float16) #/ 255.0

            if self.transform is not None:
                seed = np.random.randint(2147483647)  # Fix the seed for reproducibility
...
def get_dataloaders(root_dir,dataset_name=None, batch_size=None, transformations=None, split_size = None):
    ...
    '''
    #mean, std=compute_mean_std('data/data_modified/Dichtflächen/patched/grabs')
    ...
    trans_image1 = v2.Compose([
        v2.RandomHorizontalFlip(p=1),
        v2.PILToTensor(),
        v2.ToDtype(torch.float16, scale=True),
    ])

    trans_image1_mask = v2.Compose([
        v2.RandomHorizontalFlip(p=1),
        v2.PILToTensor(),
        v2.ToDtype(torch.float16, scale=False),
    ])
    ...
    trans2_mask = v2.Compose([
        v2.RandomVerticalFlip(p=1),
        v2.PILToTensor(),
        v2.ToDtype(torch.float16, scale=False),
    ])

    dataset_original = CustomDataset(root_dir=root_dir, dataset_name=dataset_name, transform=transformations, mask_transform=transformations_mask)
    dataset_flip = CustomDataset(root_dir=root_dir, dataset_name=dataset_name, transform=trans_image1, mask_transform=trans_image1_mask)
    dataset_distort = CustomDataset(root_dir=root_dir, dataset_name=dataset_name, transform=trans2, mask_transform=trans2_mask)
...
if __name__ == '__main__':
     try:
        ...
        #     print(f"First image min: {images[i].min()}, max: {images[i].max()}")
        #     print(f"First mask min: {masks[i].min()}, max: {masks[i].max()}")  

    #     #_____________________________________________________________
        save_name = 'test_UNetBatchNorm_Dropout'
